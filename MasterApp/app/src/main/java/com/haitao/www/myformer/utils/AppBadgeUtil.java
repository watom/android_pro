...
public class AppBadgeUtil {
    ...
    public static boolean setNotificationBadge(int count, Context context) {
        ...
        Intent intent = new Intent(context, MainActivity.class); //默认主页面调用，其他页面可修改
        PendingIntent pendingIntent = PendingIntent.getActivity(context, 0, intent, 0);
        Notification notification = new NotificationCompat.Builder(context, "badge")
                .setContentTitle(PackageUtil.getAppName(context))
                .setContentText("你有" + count + "条未读消息")
                .setLargeIcon(PackageUtil.getAppLogoBitmap(context))
                .setSmallIcon(PackageUtil.getAppLogoResourceId(context))
                .setAutoCancel(true)
    ...
    private static boolean setHonorBadge(int count, Context context) {
        try {
            ...
            if (DataUtil.isEmpty(launchClassName)) {
                return false;
            }
    ...
    private static boolean setSamsungBadge(int count, Context context) {
        try {
            ...
            if (DataUtil.isEmpty(launcherClassName)) {
                return false;
            }
